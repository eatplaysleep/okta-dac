...
module.exports.handler = (event, context) => {
	...
	if (
		requiresAccessToken.findIndex(resource => {
			return proxyPath === resource;
		}) < 0
	) {
		...
		.then(jwt => {
			...
			if (tenants && tenants.length > 0) {
				...
				tenants.forEach(tenant => {
					...
					policy.allowMethod(
						AuthPolicy.HttpVerb.PUT,
						'tenants/' + parts[1] + '/admins/*'
					); // Assign Tenant Admins
					policy.allowMethod(
						AuthPolicy.HttpVerb.POST,
						'tenants/' + parts[1] + '/domains'
					); // Register Tenant Domains
					policy.allowMethod(
						AuthPolicy.HttpVerb.PUT,
						'tenants/' + parts[1] + '/domains/*'
					...
					policy.allowMethod(
						...
						'tenants/' + parts[1] + '/domains/*'
					); // De-register Tenant Domains
					policy.allowMethod(
						AuthPolicy.HttpVerb.PUT,
						'tenants/' + parts[1] + '/apps/*'
					); // Assign all tenant users to app
				});
			}
