...
module.exports.handler = async (event, context) => {
	async function assignTenantApp(tenant, appId) {
		...
		try {
			...
			await lib.axios.get(lib.orgUrl + '/api/v1/apps/' + appId, lib.headers);
		...
		try {
			...
			const roles = await lib.axios.get(
				lib.orgUrl + '/api/v1/groups/' + tnt.data.id + '/roles',
				lib.headers
			);
