...
async function getTenantApps(tenant, claims) {
	async function filterAppsById(ids, allUsersApps) {
		...
		try {
			const res = await lib.axios.get(
				lib.orgUrl + '/api/v1/apps?limit=100',
				lib.headers
			);
			let filtered = [];
			...
			return {
				...
				data: filtered.map(app => {
					return {
						...
						settings: allUsersApps.includes(app.id)
							? { allUsers: true }
							: { allUsers: false },
					};
				}),
	...
	try {
		...
		let allUsersApps = [];
		let tenants = [];
		if (claims.tenants) tenants = JSON.parse(claims.tenants);
		for (const t of tenants) {
			const usersGroupId = t.split(':')[2];
			const res = await lib.axios.get(
				lib.orgUrl + '/api/v1/groups/' + usersGroupId + '/apps',
				lib.headers
			);
			allUsersApps = allUsersApps.concat(
				res.data.map(app => {
					return app.id;
				})
			);
