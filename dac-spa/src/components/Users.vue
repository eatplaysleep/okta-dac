...
export default {
	...
	data: () => ({
		dialog: false,
		statusFilterString:
			'status+eq+%22ACTIVE%22+or+status+eq+%22STAGED%22+or+status+eq+%22RECOVERY%22+or+status+eq+%22DEPROVISIONED%22+or+status+eq+%22SUSPENDED%22+or+status+eq+%22PROVISIONED%22+or+status+eq+%22LOCKED_OUT%22+or+status+eq+%22PASSWORD_EXPIRED%22',
		headers: [
			{
				text: 'First Name',
				value: 'firstName',
