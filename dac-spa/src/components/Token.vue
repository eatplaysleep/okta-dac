	...
	<div class="mr-4 ml-6 justify-center">
		...
		>
			<v-card
				...
				max-width="500"
				v-if="!jwtMode"
				:class="{ animated: countdown == 1, shake: countdown == 1 }"
			>
				<v-card-title class="grey justify-center mt-4"
					><h2>{{ tokenType }}</h2></v-card-title
				>
				...
				<v-list>
					...
					>
						...
						<v-list-item-content>
							...
							<v-list-item-subtitle
								class="blue--text"
								v-if="!listClaims.includes(claim)"
								>{{ getClaim(claim) }}</v-list-item-subtitle
							>
...
export default {
	...
	created() {
		let exp =
			this.type == 'o4oToken' ? this.getClaim('exp') : this.token.expiresAt;
