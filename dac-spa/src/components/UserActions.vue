...
<template>
	<div>
		...
		<div v-else>
			<template v-for="action in validActions">
				<v-btn
					...
					:key="action.type"
				>
					<v-icon :color="action.color" size="16" class="mr-2">{{
						action.icon
					}}</v-icon>
					{{ action.label }}
				</v-btn>
		...
		<v-dialog v-model="dialog" max-width="600">
			<Confirm v-on:close="dialog = false" v-on:ok="confirm" :action="action">
			</Confirm>
		</v-dialog>
	</div>
</template>
		...
		validActions() {
			...
			for (const property in this.links) {
				if (
					...
					if (
						(this.user.status == 'ACTIVE' && action.activeOnly) ||
						!action.activeOnly
					) {
						validActions.push(Object.assign(action, this.links[property]));
					}
				}
			}
			return validActions;
